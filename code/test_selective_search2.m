...
for ci=1:num_c
	for cli=1:num_classes
		...
		for ii=1:num_iter
			...
			sparse_testF = sparse(testF(start_index:end_index, :));
			testL2 = ones(size(sparse_testF, 1), 1);
			[~, ~, dec] = liblinear_predict(testL2, sparse_testF, model, ['']);
